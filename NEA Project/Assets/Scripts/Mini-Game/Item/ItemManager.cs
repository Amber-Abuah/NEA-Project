...
public class ItemManager : MonoBehaviour
{
    ...
    {
        timer.ModifyTime(item.TimeModifierAmount);
    }
    ...
    public void AddItem(Item item)
    {
        ...
        {
            // If the item picked up is a revision item, generate a random revision site link from the database
            revisionItemManager.AddRandomLink();
        }
    }
    ...
    void ApplyTimeAndScoreItemEffects()
    {
        ...
        for (int i = 0; i < items.Size; i++)
        {
            ...
            if (currentItem.GetType() == typeof(TimeModifierItem))
            {
              //  Debug.Log("Time modifier found! Apply time effects!");
                ApplyTimerEffect((TimeModifierItem) currentItem);
            }
            else if (currentItem.GetType() == typeof(ScoreModifierItem))
            {
